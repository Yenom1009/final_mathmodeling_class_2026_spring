...
from .parameter_sets import (
    discrete_map_params,
    leslie_gower_params,
    liu2021_style_params,
    main_reference_params,
    tritrophic_params,
)
from .plotting import (
    ...
    plot_discrete_extension,
    plot_fear_function_robustness,
    plot_leslie_gower_extension,
    plot_mean_amplitude,
...
def user_literature_params() -> dict[str, float]:
    return main_reference_params()


def literature_liu_params() -> dict[str, float]:
    return liu2021_style_params()
...
def make_delay_extension(root: Path, params: dict[str, float]) -> dict[str, pd.DataFrame]:
    ...
    for tau in (0.5, 2.0, 8.0):
        ...
        series[f"tau={tau}"] = df
        df.to_csv(root / "results" / f"delay_tau_{str(tau).replace('.', 'p')}.csv", index=False)
    return series


def make_leslie_gower_extension(root: Path) -> pd.DataFrame:
    params = leslie_gower_params()
    df = simulate_model("leslie_gower", params, [0.8, 0.3], T=500.0, n_points=2501)
    df.to_csv(root / "results" / "leslie_gower_time_series.csv", index=False)
    return df


def make_discrete_extension(root: Path) -> tuple[dict[str, pd.DataFrame], dict[str, list[float]]]:
    params = discrete_map_params()
    trajectories, bifurcation = simulate_discrete_map(params, [5.0, 2.0], [0.0, 5.0, 10.0, 15.0, 20.0], 500)
    for label, df in trajectories.items():
        safe = label.replace("=", "_").replace(".", "p")
        df.to_csv(root / "results" / f"discrete_{safe}.csv", index=False)
    pd.DataFrame(bifurcation).to_csv(root / "results" / "discrete_bifurcation_cloud.csv", index=False)
