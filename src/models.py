...
def model_delay_fear(
    t: float,
    state: np.ndarray,
    delayed_state: np.ndarray,
    params: Params,
) -> np.ndarray:
    """Delay extension: fear reacts to delayed predator density."""
    del t
    x, y = state
    _, y_delay = delayed_state
    r = get_param(params, "r")
    d1 = get_param(params, "d1", 0.0)
    d2 = get_param(params, "d2")
    p_pred = get_param(params, "p_pred")
    ...
    k_fear = get_param(params, "k_fear", 0.0)
    pred = p_pred * x * y / (1.0 + h_handle * x)
    dx = r * x * fear_multiplier(k_fear, y_delay, params) - d1 * x - d2 * x * x - pred
    dy = eta * pred - d3 * y
    return np.array([dx, dy], dtype=float)


def model_leslie_gower(t: float, state: np.ndarray, params: Params) -> np.ndarray:
    """Fear-augmented Leslie-Gower / Holling-III comparison model."""
    del t
    w, v = state
    r = float(params.get("lg_r", params.get("r", 0.8)))
    K = float(params.get("lg_K", 1.0))
    theta = float(params.get("lg_theta", 2.0))
    a = float(params.get("lg_a", 0.5))
    s = float(params.get("lg_s", 0.5))
    tau_pred = float(params.get("lg_tau_pred", 0.3))
    fear = 1.0 / (1.0 + theta * max(v, 0.0))
    holling3 = a * w * w * v / (w * w + s * v * v + 1e-12)
    dw = fear * r * w * (1.0 - w / K) - holling3
    dv = tau_pred * v * (1.0 - v / max(w, 1e-10))
    return np.array([dw, dv], dtype=float)


def step_discrete_fear(x: float, y: float, params: Params, k_fear: float) -> tuple[float, float]:
    """Discrete-time comparison model with fear-reduced prey growth."""
    a_growth = float(params.get("disc_a_growth", 2.0))
    d_death = float(params.get("disc_d_death", 0.1))
    b_comp = float(params.get("disc_b_comp", 0.05))
    c_pred = float(params.get("disc_c_pred", 0.03))
    mu_conv = float(params.get("disc_mu_conv", 0.5))
    e_death = float(params.get("disc_e_death", 0.2))
    fear = 1.0 / (1.0 + k_fear * max(y, 0.0))
    x_next = x * math.exp(fear * a_growth - d_death - b_comp * x - c_pred * y)
    y_next = y * math.exp(mu_conv * c_pred * x - e_death)
